...
var FcmService = /** @class */ (function () {
    function FcmService(http, fcm, db) {
        this.http = http;
        this.fcm = fcm;
        this.db = db;
    }
    FcmService.prototype.createNotification = function (firstname, lastname, idFriend, text) {
        var _this = this;
        this.db.list("/tokens/" + idFriend).valueChanges().subscribe(function (tok) {
            var token = tok.filter(function (tok) {
                return tok[0];
            });
            console.log(token);
            ...
            return _this.http.post(url, body, httpOptions).subscribe(function (res) {
                console.log(res);
            });
        });
    };
    FcmService.prototype.getAndSaveToken = function (idLoggedUser) {
        var _this = this;
        this.fcm.getToken().then(function (tok) {
            _this.db.database.ref("/tokens/" + idLoggedUser).update({
                token: tok
            });
        });
    ...
    FcmService.prototype.sendNotification = function () {
    };
...
/***/ "./src/app/services/send-message.service.ts":
/*!**************************************************!*\
  !*** ./src/app/services/send-message.service.ts ***!
  \**************************************************/
...
var SendMessageService = /** @class */ (function () {
    function SendMessageService(db) {
        this.db = db;
    }
    SendMessageService.prototype.send = function (id, uid) {
        return this.db.list("/chat/" + id + "/" + uid + "/messages/");
    };
    SendMessageService.prototype.newMessages = function (id, uid) {
        return this.db.database.ref("/chat/" + id + "/" + uid + "/");
    };
    SendMessageService.ctorParameters = function () { return [
        { type: _angular_fire_database__WEBPACK_IMPORTED_MODULE_1__["AngularFireDatabase"] }
    ]; };
    SendMessageService = tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"]([
